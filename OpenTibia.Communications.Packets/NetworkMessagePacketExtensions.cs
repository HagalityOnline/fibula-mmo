...
namespace OpenTibia.Communications.Packets
{
    ...
    public static class NetworkMessagePacketExtensions
    {
            ...
            for (var i = 0; i < numberOfMovements; i++)
            {
                ...
                switch (dir)
                {
                    ...
                    case 8:
                        ...
                        break;
                    default:
                        throw new InvalidDataException($"Invalid direction value {dir} on message.");
                }
            }
        ...
        {
            return new DefaultReadPacket(message.GetBytes(message.Length - message.Position));
        }
        ...
        /// <summary>
        /// Reads player login information sent in the message.
        /// </summary>
        /// <param name="message">The mesage to read the information from.</param>
        /// <returns>The player login information.</returns>
        public static IPlayerLoginInfo ReadPlayerLoginInfo(this INetworkMessage message)
        {
        ...
        public static void WriteCreatureLightPacket(this INetworkMessage message, CreatureLightPacket packet)
        {
            ...
            message.AddUInt32(packet.Creature.Id);
            message.AddByte(packet.Creature.LightBrightness); // light level
            message.AddByte(packet.Creature.LightColor); // color
        }
        ...
        public static void WriteMapDescriptionPacket(this INetworkMessage message, MapDescriptionPacket packet)
        {
            ...
            message.AddBytes(packet.DescriptionBytes); // TODO: change this?
        }
        ...
        public static void WritePlayerSkillsPacket(this INetworkMessage message, PlayerSkillsPacket packet)
        {
            ...
            message.AddByte(packet.Player.GetSkillInfo(SkillType.Fist));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Fist));

            message.AddByte(packet.Player.GetSkillInfo(SkillType.Club));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Club));

            message.AddByte(packet.Player.GetSkillInfo(SkillType.Sword));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Sword));

            message.AddByte(packet.Player.GetSkillInfo(SkillType.Axe));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Axe));

            message.AddByte(packet.Player.GetSkillInfo(SkillType.Ranged));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Ranged));

            message.AddByte(packet.Player.GetSkillInfo(SkillType.Shield));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Shield));

            message.AddByte(packet.Player.GetSkillInfo(SkillType.Fishing));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Fishing));
        }

        /// <summary>
        /// Writes the contents of the <see cref="PlayerStatusPacket"/> into the message.
        /// </summary>
        /// <param name="message">The message to write to.</param>
        /// <param name="packet">The packet to write in the message.</param>
        public static void WritePlayerStatusPacket(this INetworkMessage message, PlayerStatusPacket packet)
        {
            packet.ThrowIfNull(nameof(packet));
            ...
            message.AddUInt32(Math.Min(0x7FFFFFFF, packet.Player.Experience)); // Experience: Client debugs after 2,147,483,647 exp

            message.AddUInt16(packet.Player.Level);
            message.AddByte(packet.Player.LevelPercent);
            ...
            message.AddByte(packet.Player.GetSkillInfo(SkillType.Magic));
            message.AddByte(packet.Player.GetSkillPercent(SkillType.Magic));
        ...
        public static void WritePlayerWalkCancelPacket(this INetworkMessage message, PlayerWalkCancelPacket packet)
        {
            ...
            message.AddByte((byte)packet.Direction);
        }
        ...
        public static void WriteSelfAppearPacket(this INetworkMessage message, SelfAppearPacket packet)
        {
            ...
            message.AddByte(Math.Min((byte)0x01, packet.Player.AccessLevel));

            if (packet.Player.AccessLevel > 0)
            {
