...
namespace OpenTibia.Communications.Handlers.Game
{
    ...
    public class ItemUseHandler : GameHandler
    {
        ...
        public ItemUseHandler(IGame gameInstance)
            : base(gameInstance)
        {
        }
        ...
        public override void HandleRequest(INetworkMessage message, IConnection connection)
        {
            ...
            if (itemUseInfo.FromLocation.Type == LocationType.Ground)
            {
                ...
                if (locationDiff.MaxValueIn2D > 1)
                {
                    ...
                    var directions = this.Game.Pathfind(player.Location, itemUseInfo.FromLocation, out Location retryLoc).ToArray();

                    player.SetPendingAction(new UseItemPlayerAction(player, itemUseInfo, retryLoc));
            ...
            new UseItemPlayerAction(player, itemUseInfo, itemUseInfo.FromLocation).Perform();
        }
    }
}